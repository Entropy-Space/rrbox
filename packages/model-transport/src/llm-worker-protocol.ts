...
export const LLM_WORKER_PROTOCOL_VERSION = 1 as const;
...
export type LlmWorkerCommand =
  | CommandEnvelope<"stream_start", { model_request: ModelRequest }>
  | CommandEnvelope<"stream_abort", Record<string, never>>;
...
export function parseLlmWorkerCommand(value: unknown): LlmWorkerCommand {
  ...
  const streamId = requireIdentifier(envelope.stream_id, "stream_id");
  ...
  switch (envelope.type) {
    ...
    case "stream_abort":
      return createLlmStreamAbort(streamId);
    default:
